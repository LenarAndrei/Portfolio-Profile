...
export default function About() {
  ...
  return (
    <section id="about" className="py-20 md:py-28 lg:py-32 relative overflow-hidden">
      {/* Modern Background */}
      ...
      <div className="container mx-auto px-6 sm:px-8 lg:px-12 relative z-10 max-w-7xl">
        <motion.div
          ...
          viewport={{ once: true }}
          className="text-center mb-16 md:mb-20 lg:mb-24 flex flex-col items-center justify-center"
        >
          <motion.div 
            className="inline-block p-3 lg:p-4 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-2xl mb-8"
            whileHover={{ scale: 1.05 }}
          >
            <span className="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-semibold rounded-xl">About Me</span>
          </motion.div>
          <h2 className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black mb-8 lg:mb-12 bg-gradient-to-r from-gray-900 via-purple-800 to-gray-900 dark:from-white dark:via-purple-200 dark:to-white bg-clip-text text-transparent px-4">
            Crafting Digital Excellence
          </h2>
          <p className="text-xl lg:text-2xl text-gray-600 dark:text-gray-300 max-w-4xl mx-auto leading-relaxed px-4">
            Transforming ideas into powerful digital solutions through innovative design and development
          </p>
        </motion.div>

        <div className="max-w-6xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12 md:gap-16 lg:gap-20 items-center justify-center mb-16 md:mb-20 lg:mb-24">
            <motion.div
              initial={{ opacity: 0, x: -50 }}
              whileInView={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.8 }}
              viewport={{ once: true }}
              className="backdrop-blur-xl bg-white/60 dark:bg-gray-900/60 p-10 lg:p-16 rounded-3xl shadow-2xl border border-white/20 mx-auto w-full"
            >
            ...
            <div className="space-y-6 lg:space-y-8 text-gray-600 dark:text-gray-300 text-lg leading-relaxed">
              <p>
          ...
          >
            {skills.map((skill, index) => (
              <motion.div
                ...
                className="group p-8 lg:p-10 bg-gradient-to-br from-white/80 to-gray-50/80 dark:from-gray-800/80 dark:to-gray-700/80 backdrop-blur-xl rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-300 border border-white/30"
              >
                <div className="flex items-center mb-8">
                  <motion.div 
                    className="p-4 bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl mr-6"
                    whileHover={{ rotate: 10 }}
                  >
                    <skill.icon className="w-8 h-8 text-white" />
                  </motion.div>
                  <h4 className="text-2xl lg:text-3xl font-bold text-gray-800 dark:text-gray-200">
                    {skill.category}
                  </h4>
                </div>
                <div className="flex flex-wrap gap-3">
                  {skill.items.map((item) => (
                    <motion.span 
                      key={item} 
                      whileHover={{ scale: 1.05 }}
                      className="px-4 py-3 bg-gradient-to-r from-blue-500/10 to-purple-500/10 hover:from-blue-500/20 hover:to-purple-500/20 text-gray-700 dark:text-gray-300 rounded-full text-sm lg:text-base font-medium border border-blue-500/20 transition-all duration-300"
                    >
                      {item}
                    </motion.span>
                  ))}
